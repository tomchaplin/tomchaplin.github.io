...
If you would like to download the Python file which controls the quadcopter in Kerbal Space Program please [click here](https://github.com/tomchaplin/KSP_Control_Systems/blob/master/pid_control.py). To use this script you will need to have Python installed locally and the [kRPC](https://krpc.github.io/krpc/index.html) mod installed in your instance of KSP. Also, so that the script can properly detect the engines, you will need to set an initial thrust limiter on the engines where engine \\(T_i\\) has a thrust limiter of \\(i/10\\).
